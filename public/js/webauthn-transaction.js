...
class e extends HTMLElement {
    constructor() {
        super(), this.root = this.attachShadow({mode: "open"}), this._onFormSubmitListener = this._onFormSubmit.bind(this), this.assertionStartUrl = SERVER + "/api/transaction/start", this.assertionFinishUrl = SERVER + "/api/transaction/finish", this.fetchOptions = {
            method: "POST",
            credentials: "include",
    ...
    connectedCallback() {
        this.update(), this.root.querySelector("#confirm-button-yes").addEventListener("click", this._onFormSubmitListener);
    ...
    async _onFormSubmit(t) {
        try {
            ...
            if (!n.ok) throw new Error("Could not successfuly start transaction");
